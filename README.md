...
| <a name="input_cluster_endpoints"></a> [cluster\_endpoints](#input\_cluster\_endpoints) | A map of additional cluster endpoints and their attributes to be created | <pre>map(object({<br>    excluded_members = optional(list(string), [])<br>    static_members   = optional(list(string), [])<br>    type             = string<br>  }))</pre> | `{}` | no |
