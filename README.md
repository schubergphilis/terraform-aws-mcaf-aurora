...
| <a name="input_cluster_endpoints"></a> [cluster\_endpoints](#input\_cluster\_endpoints) | A map of additional cluster endpoints and their attributes to be created | <pre>map(object({<br>    excluded_members = optional(list(string), [])<br>    static_members   = optional(list(string), [])<br>    type             = string<br>  }))</pre> | `null` | no |
...
| <a name="input_security_group_rules"></a> [security\_group\_rules](#input\_security\_group\_rules) | Map of security group rules to add to the cluster security group created | <pre>object({<br>    ingress_allowed_cidr_blocks        = optional(list(string), null)<br>    ingress_allowed_security_group_ids = optional(list(string), null)<br>  })</pre> | `null` | no |
